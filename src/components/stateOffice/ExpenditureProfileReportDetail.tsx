...
import {
  REPORT_CONFIG, EXPENDITURE_SUB_HEADS,
  monthLabel, quarterFromMonth, labelOf, formatNaira, formatDate, calcExpenditurePct,
} from "./constants";
...
export default function ExpenditureProfileReportDetail({ reportId, onBack, onEdit }: Props) {
                            ...
                            {lines.map((line, i) => (
                              <TableRow key={String(line.id ?? i)} className="hover:bg-[#f8fdfb]">
                                ...
                                <TableCell className="text-sm font-medium">
                                  {labelOf(EXPENDITURE_SUB_HEADS, String(line.sub_head ?? ""))}
                                </TableCell>
                                <TableCell className="text-sm text-right tabular-nums">
                                  {calcExpenditurePct(Number(line.amount), totalAmount).toFixed(2)}%
                                </TableCell>
                                <TableCell className="text-sm text-right tabular-nums">{formatNaira(line.amount as number)}</TableCell>
                              </TableRow>
                            ))}
